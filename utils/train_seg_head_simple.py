...
VAL_SPLIT = 0.2
NICKNAME = "simple_segformer_head_only"
CHECKPOINT_DIR = Path("ckpts_classification") / NICKNAME
BEST_CHECKPOINT_PATH = CHECKPOINT_DIR / "segformer_head_only_best.pt"
...
def freeze_segformer_backbone(model: SegFormerRisk) -> None:
...
def main() -> None:
    ...
    freeze_segformer_backbone(model)
    trainable_parameters = sum(parameter.numel() for parameter in model.parameters() if parameter.requires_grad)
    print(f"Trainable parameters (head only): {trainable_parameters:,}")
    ...
    try:
        for epoch in range(1, NUM_EPOCHS + 1):
            ...
            epoch_ckpt = CHECKPOINT_DIR / f"segformer_head_only_epoch_{epoch:03d}.pt"
            save_checkpoint(
                model=model,
                optimizer=optimizer,
                scheduler=scheduler,
