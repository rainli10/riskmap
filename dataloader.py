...
RISK_BIN_EDGES = (0.2, 0.4, 0.6, 0.8)
RISK_CLASS_VALUES = (0.1, 0.3, 0.5, 0.7, 0.9)
RISK_CLASS_LABELS = (
    "[0.0, 0.2)",
    "[0.2, 0.4)",
    "[0.4, 0.6)",
...
def quantize_risk_map(
    risk_map: np.ndarray,
    bin_edges: Sequence[float] = RISK_BIN_EDGES,
) -> np.ndarray:
    """Map continuous risk values in [0, 1] to discrete class ids."""
    risk = np.clip(np.asarray(risk_map, dtype=np.float32), 0.0, 1.0)
    edges = np.asarray(tuple(bin_edges), dtype=np.float32)
    return np.digitize(risk, bins=edges, right=False).astype(np.int64)


def risk_classes_to_values(
    class_map: np.ndarray,
    class_values: Sequence[float] = RISK_CLASS_VALUES,
) -> np.ndarray:
    """Map discrete risk classes back to representative continuous values."""
    classes = np.asarray(class_map, dtype=np.int64)
    values = np.asarray(tuple(class_values), dtype=np.float32)

    if classes.size == 0:
        return np.asarray(classes, dtype=np.float32)

    if np.any(classes < 0) or np.any(classes >= len(values)):
        raise ValueError(
            f"Risk classes must be in [0, {len(values) - 1}], got range "
            f"{int(classes.min())} to {int(classes.max())}."
        )

    return values[classes]


def build_binned_risk_map(
    ...
    """Construct a blocked risk map and quantize it into discrete classes."""
    continuous_risk = build_blocked_risk_map(
        label_map=label_map,
        depth_map=depth_map,
    ...
    return quantize_risk_map(continuous_risk, bin_edges=bin_edges)
...
def connected_components(binary_mask: np.ndarray, connectivity: int = 8) -> tuple[np.ndarray, int]:
    ...
    # Fast C++ implementation when OpenCV is available.
    try:
        cv2_module = importlib.import_module("cv2")
        num_labels, labels = cv2_module.connectedComponents(
            mask.astype(np.uint8),
            connectivity=connectivity,
        )
        component_map = labels.astype(np.int32) - 1
        return component_map, int(max(num_labels - 1, 0))
    except Exception:
        pass
...
def build_blocked_risk_map(
    ...
    # Avoid a global object map and full-image masking per component.
    for class_id in unique_labels:
        class_mask = label_ids == int(class_id)
        component_map, num_components = connected_components(class_mask, connectivity=connectivity)

        for component_id in range(num_components):
            object_mask = component_map == component_id
            valid_object_depths = depth[object_mask & valid_depth_mask]
    ...
    return np.clip(blocked_risk_map, 0.0, 1.0).astype(np.float32)
